...
var MJr = {
    ...
    initWidget(widgetID, width, height, modelFunc) {
        ...
        canvasElem.height = height * pixelScale;
        divElem.appendChild(canvasElem);
        
        let resetBtnElem = document.createElement('button');
        resetBtnElem.innerText = 'Reset';
        divElem.appendChild(resetBtnElem);
        ...
        function reset() {
            ...
            iter = modelFunc(width, height);
            update();
        }
        function update() {
            let r = iter.next();
            if(r.done) { return; }
            let grid = r.value.grid;
            for(let dy = 0; dy < r.value.height; ++dy) {
                let y = r.value.y + dy;
                for(let dx = 0; dx < r.value.width; ++dx) {
                    let x = r.value.x + dx;
                    let c = grid.alphabet[grid.data[x + y * grid.width]];
                    ctx.fillStyle = MJr.palette[c];
                    ctx.fillRect(x * pixelScale, y * pixelScale, pixelScale, pixelScale);
                }
            }
            requestAnimationFrame(update);
        }
        resetBtnElem.onclick = reset;
        reset();
    },
};
